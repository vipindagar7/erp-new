...
// ⚠️  Bulk submit is ROOT ADMIN ONLY — do not show this component to ADMIN or SUPER_ADMIN
export default function BulkFeedbackUpload({ selectedFormId = "", forms = [], isRoot: isRootProp = false }) {
  // Read is_root directly from Redux — don't rely only on prop
  const isRootRedux = useSelector((s) => s.auth?.user?.is_root ?? false);
  const isRoot = isRootProp || isRootRedux;
  ...
  if (!isRoot) {
    return (
      <div className="text-center py-16 bg-card border border-dashed border-destructive/30 rounded-2xl">
        <p className="font-medium text-destructive">Root Admin Only</p>
        <p className="text-sm text-muted-foreground mt-1">Bulk submission is restricted to root administrators only.</p>
      </div>
    );
  }
