...
export default function ProgramHistoryPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await axiosInstance.get(EP.audit.list, { params: { module: "program", page, limit } });
      const d = r.data?.data;
      setLogs(d?.logs || d || []);
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(ROUTES.programs.hub)}
          className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground">
          <ArrowLeft size={18} />
        </button>
        <div>
          <h1 className="text-xl font-bold">Program History</h1>
