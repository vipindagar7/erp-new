// src/modules/student/pages/StudentActivityPage.jsx
...
export default function StudentActivityPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await axiosInstance.get(`${EP.students.byId(id)}/activity`, { params: { page, limit } });
      const d = r.data?.data; setLogs(d?.logs || []); setTotal(d?.pagination?.total ?? 0);
    } catch { notify.error("Failed"); } finally { setLoading(false); }
  };
  useEffect(() => { axiosInstance.get(EP.students.byId(id)).then((r) => setName(r.data?.data?.name || "")); }, [id]);
  useEffect(() => { load(); }, [page]);
  return (
