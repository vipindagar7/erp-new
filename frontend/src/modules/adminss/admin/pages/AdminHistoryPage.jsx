// src/modules/admin/pages/AdminHistoryPage.jsx
...
export default function AdminHistoryPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await axiosInstance.get(EP.audit.list, { params: { module: "admin", page, limit } });
      const d = r.data?.data;
      setLogs(d?.logs || d || []);
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-xl font-bold">Admin History</h1><p className="text-sm text-muted-foreground">{total} audit entries — every action on every admin account</p></div>
      </div>
