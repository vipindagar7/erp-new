// backend/modules/enrollment/enrollment.service.js
// Hierarchy: Department → Program → Branch → Section
// course_id removed everywhere — replaced by program_id + branch_id
...
const enrollInclude = {
  ...
  section: {
    select: {
      ...
      branch: { select: { name: true, program: { select: { name: true } } } },
    },
  },
};

// ── List for one student ──────────────────────────────────────
export const getStudentEnrollments = (student_id) =>
...
export const getAllEnrollments = async ({
  page = 1, limit = 20, search, section_id, dept_id, program_id, branch_id,
  ...
  const [enrollments, total] = await Promise.all([
    prisma.studentEnrollment.findMany({ where, skip: (page - 1) * limit, take: limit, include: enrollInclude, orderBy: [{ is_current: "desc" }, { enrolled_at: "desc" }] }),
...
export const createEnrollment = async (student_id, data) => {
  const student = await prisma.student.findUnique({
    where:  { id: student_id },
    select: { id: true, dept_id: true, program_id: true, branch_id: true, section_id: true },
  });
  if (!student) throw Object.assign(new Error("Student not found"), { status: 404 });

  if (data.is_current) {
    await prisma.studentEnrollment.updateMany({ where: { student_id, is_current: true }, data: { is_current: false } });
  }
  ...
  return prisma.studentEnrollment.create({
    data: {
      ...
      branch_id:     data.branch_id     || student.branch_id || null,
...
export const updateEnrollment = async (enrollment_id, data) => {
  ...
  if (data.is_current && !enrollment.is_current) {
    await prisma.studentEnrollment.updateMany({ where: { student_id: enrollment.student_id, is_current: true }, data: { is_current: false } });
  }
  ...
  return prisma.studentEnrollment.update({ where: { id: enrollment_id }, data: updateData, include: enrollInclude });
};
...
export const deleteEnrollment = async (enrollment_id) => {
  ...
  if (e.is_current) throw Object.assign(new Error("Cannot delete the current enrollment. Set another as current first."), { status: 400 });
...
export const setCurrentEnrollment = async (enrollment_id) => {
  const e = await prisma.studentEnrollment.findUnique({ where: { id: enrollment_id } });
  if (!e) throw Object.assign(new Error("Enrollment not found"), { status: 404 });
  await prisma.studentEnrollment.updateMany({ where: { student_id: e.student_id, is_current: true }, data: { is_current: false } });
  return prisma.studentEnrollment.update({ where: { id: enrollment_id }, data: { is_current: true }, include: enrollInclude });
};
...
export const bulkCreateEnrollments = async (buffer) => {
  ...
  for (const row of rows) {
    ...
    const academic_year = String(row["Academic Year*"]  || "").trim();
    const semester      = parseInt(row["Semester*"]     || 0);
    const status        = String(row["Status"] || "ACTIVE").trim().toUpperCase();
    const is_current    = String(row["Is Current"] || "").toLowerCase() === "true";
    const section_id    = String(row["Section ID"] || "").trim() || null;
    const branch_id     = String(row["Branch ID"]  || "").trim() || null;
    const batch_year    = parseInt(row["Batch Year"] || 0) || 0;
    const remarks       = String(row["Remarks"] || "").trim() || null;

    if (!student_id || !academic_year || !semester) { results.failed.push({ row: student_id || "(empty)", reason: "Student ID, Academic Year and Semester required" }); continue; }

    try {
      const student = await prisma.student.findUnique({
        where:  { id: student_id },
        select: { id: true, name: true, roll_no: true, dept_id: true, program_id: true, branch_id: true, section_id: true },
      });
      if (!student) { results.failed.push({ row: student_id, reason: "Student not found" }); continue; }

      const dup = await prisma.studentEnrollment.findFirst({ where: { student_id, academic_year, semester } });
      ...
      const enrollment = await prisma.studentEnrollment.create({
        data: {
          ...
          branch_id:    branch_id || student.branch_id || null,
          academic_year, semester, batch_year, status, is_current, remarks,
        },
      });
      results.created.push({ id: enrollment.id, student_id, name: student.name });
    } catch (e) { results.failed.push({ row: student_id, reason: e.message }); }
  }
  return results;
};
...
export const bulkDeleteEnrollments = async (enrollment_ids) => {
  const currents = await prisma.studentEnrollment.findMany({ where: { id: { in: enrollment_ids }, is_current: true }, select: { id: true } });
  if (currents.length > 0) throw Object.assign(new Error(`Cannot delete ${currents.length} current enrollment(s).`), { status: 400 });
  const result = await prisma.studentEnrollment.deleteMany({ where: { id: { in: enrollment_ids } } });
...
export const generateTemplate = async () => {
  const students = await prisma.student.findMany({
    take: 5,
    select: { id: true, name: true, roll_no: true, section: { select: { id: true, name: true } } },
    orderBy: { name: "asc" },
  });
  const headers = [
    ...
    "Is Current (true/false)", "Section ID", "Branch ID", "Remarks",
  ];
...
export const exportEnrollments = async (filters = {}) => {
  ...
  const enrollments = await prisma.studentEnrollment.findMany({
    where,
    include: {
      student: { select: { name: true, roll_no: true, enrollment_no: true, department: { select: { name: true } }, program: { select: { name: true } }, branch: { select: { name: true } } } },
      section: { select: { name: true, batch: true } },
    },
    orderBy: [{ academic_year: "desc" }, { semester: "asc" }, { student: { name: "asc" } }],
  });
  const headers = ["Student ID", "Name", "Roll No", "Enrollment No", "Department", "Program", "Branch", "Section", "Semester", "Academic Year", "Batch Year", "Status", "Is Current", "Enrolled At", "Completed At", "Remarks"];
  const rows = enrollments.map((e) => [
    e.student_id, e.student?.name || "", e.student?.roll_no || "", e.student?.enrollment_no || "",
    e.student?.department?.name || "", e.student?.program?.name || "", e.student?.branch?.name || "",
    e.section?.name || "", e.semester, e.academic_year, e.batch_year,
    e.status, e.is_current ? "Yes" : "No",
    new Date(e.enrolled_at).toLocaleDateString(),
    e.completed_at ? new Date(e.completed_at).toLocaleDateString() : "", e.remarks || "",
  ]);
  const wb = xlsx.utils.book_new();
