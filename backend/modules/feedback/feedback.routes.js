...
import { authenticate, authorize, rootOnly } from "../../middlewares/auth.middleware.js";
...
router.get(   "/forms/:id/bulk-template",  authenticate, rootOnly, c.getFormTemplate);
// ↓ Individual submit — all authenticated users
router.post(  "/forms/:id/submit",       authenticate, authorize(...ALL),   c.submitFeedback);
// ↓ Bulk submit — ROOT ONLY (not superadmin, not admin)
router.post(  "/forms/:id/bulk-submit",  authenticate, rootOnly, upload.single("file"), c.bulkSubmit);
...
// ↓ Group bulk submit — ROOT ONLY
router.post( "/groups/:id/bulk-submit",  authenticate, rootOnly, upload.single("file"), c.bulkSubmitGroup);
